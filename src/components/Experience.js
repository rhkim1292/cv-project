...
const Experience = () => {
	...
	const [addExpMode, setAddExpMode] = useState(false);
	...
	const startAddExpMode = (e) => {
		setAddExpMode(true);
	};

	const stopAddExpMode = (e) => {
		setAddExpMode(false);
	};
	...
	const handleChange = (e) => {
		...
		if (e.target.checked) {
			...
			setIsCurrWorking(true);
			return;
		}
		endMonthSelect.disabled = false;
		endYearSelect.disabled = false;
		setIsCurrWorking(false);
	};
		...
		>
				...
				<div key={exp_idx} className="exp-container">
						...
						<div className="edit-task-container">
							<form
								...
								onSubmit={(e) => {
									...
									currExp.tasks.push(formData.get('task'));
									setExps(exps);
					...
					{editMode ? (
						<button
							...
							onClick={(e) => {
								setExps(
									exps.filter((value, index) => {
										if (index !== exp_idx) return value;
										return null;
									})
								);
							}}
			...
			{editMode && addExpMode ? (
					...
					onSubmit={(e) => {
						...
						setExps(
							exps.concat({
								...
								endMonth: isCurrWorking
									? ''
									: formData.get('end_month'),
								endYear: isCurrWorking
									? 'Present'
				...
				>
					...
					<div className="form-row">
						<label className="checkbox-label" htmlFor="currWorking">
							<input
								id="currWorking"
								name="curr_working"
								type="checkbox"
			...
			{editMode && !addExpMode ? (
				<button className="add-exp-btn" onClick={startAddExpMode}>
					Add Experience
				</button>
			) : null}

			{editMode ? (
				<button className="exp-submit-btn" onClick={stopEditMode}>
					Submit
				</button>
			) : null}

			{hovering && !editMode ? (
				<button className="exp-edit-btn" onClick={startEditMode}>
					Edit
				</button>
			) : null}
		</div>
